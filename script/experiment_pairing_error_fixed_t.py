...
from utils.efm import efm_closed_form_weights_and_u, efm_closed_form_weights_and_u_downsample_2x
...
def compute_pairing_error_at_t(
    ...
    with torch.no_grad():
        ...
        if pairing_method == 'cfm':
            ...
            # EFM 2x 下采样：xt 用随机配对，ut 用 2x 下采样空间距离的 softmax 加权
            x0_paired, x1_paired = get_paired_samples_cfm(x0, x1)
        elif pairing_method == 'efm_down2_ot':
            # EFM 2x 下采样：xt 用 OT 配对，ut 用 2x 下采样空间距离的 softmax 加权
            x0_paired, x1_paired = get_paired_samples_otcfm(x0, x1)
        else:
...
def experiment(args):
    ...
    pairing_methods = ['otcfm', 'ma_tcfm', 'dct_otcfm_4x4', 'dct_otcfm_8x8', 'dct_otcfm_16x16', 'dct_hf_otcfm_4x4', 'dct_hf_otcfm_8x8', 'dct_hf_otcfm_16x16', 'dct_mf_otcfm_4x4', 'dct_mf_otcfm_8x8', 'efm_closed_form', 'efm_ot', 'efm_down2_cfm', 'efm_down2_ot', 'cfm']  # OTCFM在前，CFM在最后（最右边）
    # 只有当提供了双塔 checkpoint 时才启用 dual_tower 方法，避免未训练/未保存时直接报错
    if dual_tower is not None:
        pairing_methods.insert(-1, 'dual_tower')
    ...
    method_labels = {
        ...
        'efm_ot': 'EFM (OT pair)',
        'efm_down2_cfm': 'EFM Down2x (CFM pair)',
        'efm_down2_ot': 'EFM Down2x (OT pair)',
        'dual_tower': 'DualTower (InfoNCE)'
    }
    colors = ['green', 'red', 'blue', 'cyan', 'magenta', 'orange', 'brown', 'pink', 'purple', 'yellow', 'olive', 'navy', 'teal', 'coral', 'black', 'gray']  # EFM Down2x 等
