...
def get_paired_samples_dct_3l_otcfm(x0, x1):
    """DCT_3L_OTCFM: 使用DCT变换提取前三个最低频系数，然后进行OT配对
    
    对x0和x1进行DCT变换，按照zigzag扫描顺序提取前三个最低频系数（dct_1d[0:3]），然后进行OT配对。
    ...
    # 对每个通道和每个样本分别做2D DCT变换并转换为1D zigzag顺序，取前三个最低频
    x0_coeffs = []
    for i in range(B0):
        sample_coeffs = []
        for c in range(C):
            ...
            x_channel_dct = dctn(x_channel, norm='ortho')  # (H, W)
            dct_1d = _zigzag_to_1d(x_channel_dct)  # (H*W,)
            dct_first = dct_1d[0:3]  # 取前三个最低频
            sample_coeffs.append(torch.from_numpy(dct_first).to(x0.device))
        x0_coeffs.append(torch.cat(sample_coeffs, dim=0))  # (C * 3,)
    x0_flat = torch.stack(x0_coeffs, dim=0)  # (B0, C * 3)
    ...
    for i in range(B1):
        ...
        for c in range(C):
            ...
            dct_1d = _zigzag_to_1d(x_channel_dct)  # (H*W,)
            dct_first = dct_1d[0:3]  # 取前三个最低频
            sample_coeffs.append(torch.from_numpy(dct_first).to(x1.device))
        x1_coeffs.append(torch.cat(sample_coeffs, dim=0))  # (C * 3,)
    x1_flat = torch.stack(x1_coeffs, dim=0)  # (B1, C * 3)
    
    # 计算cost matrix并使用匈牙利算法配对
    M_cost = torch.cdist(x0_flat, x1_flat) ** 2
...
def get_paired_samples_dct_3h_otcfm(x0, x1):
    """DCT_3H_OTCFM: 使用DCT变换提取最后三个最高频系数，然后进行OT配对
    
    对x0和x1进行DCT变换，按照zigzag扫描顺序提取最后三个最高频系数（dct_1d[-3:]），然后进行OT配对。
    ...
    # 对每个通道和每个样本分别做2D DCT变换并转换为1D zigzag顺序，取最后三个最高频
    x0_coeffs = []
    for i in range(B0):
        sample_coeffs = []
        for c in range(C):
            ...
            x_channel_dct = dctn(x_channel, norm='ortho')  # (H, W)
            dct_1d = _zigzag_to_1d(x_channel_dct)  # (H*W,)
            dct_last = dct_1d[-3:]  # 取最后三个最高频
            sample_coeffs.append(torch.from_numpy(dct_last).to(x0.device))
        x0_coeffs.append(torch.cat(sample_coeffs, dim=0))  # (C * 3,)
    x0_flat = torch.stack(x0_coeffs, dim=0)  # (B0, C * 3)
    ...
    for i in range(B1):
        ...
        for c in range(C):
            ...
            dct_1d = _zigzag_to_1d(x_channel_dct)  # (H*W,)
            dct_last = dct_1d[-3:]  # 取最后三个最高频
            sample_coeffs.append(torch.from_numpy(dct_last).to(x1.device))
        x1_coeffs.append(torch.cat(sample_coeffs, dim=0))  # (C * 3,)
    x1_flat = torch.stack(x1_coeffs, dim=0)  # (B1, C * 3)
...
def get_paired_samples_dct_3m_otcfm(x0, x1):
    """DCT_3M_OTCFM: 使用DCT变换提取中间三个频率系数，然后进行OT配对
    
    对x0和x1进行DCT变换，按照zigzag扫描顺序提取中间三个频率系数，然后进行OT配对。
    ...
    total_coeffs = H * W
    mid_idx = total_coeffs // 2
    
    # 对每个通道和每个样本分别做2D DCT变换并转换为1D zigzag顺序，取中间三个频率
    x0_coeffs = []
    for i in range(B0):
        sample_coeffs = []
        for c in range(C):
            ...
            x_channel_dct = dctn(x_channel, norm='ortho')  # (H, W)
            dct_1d = _zigzag_to_1d(x_channel_dct)  # (H*W,)
            dct_mid = dct_1d[mid_idx-1:mid_idx+2]  # 取中间三个频率（中间位置前后各一个）
            sample_coeffs.append(torch.from_numpy(dct_mid).to(x0.device))
        x0_coeffs.append(torch.cat(sample_coeffs, dim=0))  # (C * 3,)
    x0_flat = torch.stack(x0_coeffs, dim=0)  # (B0, C * 3)
...
def compute_pairing_error_at_t(model, flow_matcher, x0, x1, pairing_method, device, t_value, epsilon, metric='cos'):
    ...
    with torch.no_grad():
        ...
        elif pairing_method == 'dct_3l_otcfm':
            x0_paired, x1_paired = get_paired_samples_dct_3l_otcfm(x0, x1)
        elif pairing_method == 'dct_3h_otcfm':
            x0_paired, x1_paired = get_paired_samples_dct_3h_otcfm(x0, x1)
        elif pairing_method == 'dct_3m_otcfm':
            x0_paired, x1_paired = get_paired_samples_dct_3m_otcfm(x0, x1)
        else:
...
def experiment(args):
    ...
    pairing_methods = ['otcfm', 'ma_tcfm', 'dct_otcfm_4x4', 'dct_hf_otcfm_4x4', 'dct_mf_otcfm_4x4', 'dct_3l_otcfm', 'dct_3h_otcfm', 'dct_3m_otcfm', 'cfm']  # OTCFM在前，CFM在最后（最右边）
    ...
    method_labels = {
        ...
        'dct_3l_otcfm': 'DCT_3L_OTCFM',
        'dct_3h_otcfm': 'DCT_3H_OTCFM',
        'dct_3m_otcfm': 'DCT_3M_OTCFM'
    }
    colors = ['green', 'red', 'blue', 'cyan', 'magenta', 'orange', 'brown', 'pink', 'gray']  # 颜色对应pairing_methods
