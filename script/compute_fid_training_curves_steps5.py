#!/usr/bin/env python3
"""
计算训练过程中每个checkpoint的FID分数（使用5 steps），并绘制训练曲线
"""
import subprocess
import sys
import json
...
# 要计算的checkpoint迭代次数
iterations = [10000, 50000, 100000, 200000, 300000, 400000]
...
def main():
    """主函数"""
    print("="*60)
    print("计算训练过程中的FID分数 (使用 5 steps)")
    print("="*60)
    ...
    for model_key, model_info in models.items():
        ...
        for iter_num in iterations:
            ...
            output_dir = Path(f"exp/fid_results/{model_key}_iter_{iter_num}_steps5")
    ...
    # 保存结果到JSON文件
    results_file = Path("exp/fid_training_curves_results_steps5.json")
    ...
    # 保存图片
    plot_file = Path("exp/fid_training_curves_steps5.png")
