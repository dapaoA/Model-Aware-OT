...
- 用不同配对方式计算配对组合
- 在固定时间t下（0.05, 0.10, 0.25, 0.5, 0.75, 0.95）
...
from train_noise_image_infonce import DualTowerInfoNCE
from utils.efm import (
    efm_closed_form_weights_and_u,
    efm_closed_form_weights_and_u_downsample_2x,
    lsefm_block_weights_and_u,
)


# 固定的时间t值
FIXED_T_VALUES = [0.00, 0.05, 0.10, 0.25, 0.5, 0.75, 0.95]
...
def get_paired_samples_dual_tower(x0, x1, dual_tower: DualTowerInfoNCE):
    """DualTower: 用双塔 embedding 空间的欧氏距离做 OT(Hungarian) 配对

    Args:
        x0: noise batch (B, C, H, W)
        x1: image batch (B, C, H, W) (CIFAR-10 normalized, same as training/infer)
        dual_tower: 训练好的 DualTowerInfoNCE 模型（只使用其 noise_encoder / image_encoder）
    """
    import torch.nn.functional as F

    dual_tower.eval()
    with torch.no_grad():
        # Normalize embeddings so Euclidean distance is well-scaled (CLIP-style)
        z0 = F.normalize(dual_tower.noise_encoder(x0), dim=-1)  # (B, D)
        z1 = F.normalize(dual_tower.image_encoder(x1), dim=-1)  # (B, D)

        M_cost = torch.cdist(z0, z1) ** 2
...
def compute_pairing_error_at_t(
    model,
    flow_matcher,
    x0,
    x1,
    ...
    epsilon,
    metric='cos',
    dual_tower: DualTowerInfoNCE | None = None,
):
    """
    在固定时间t下计算配对误差
    ...
    with torch.no_grad():
        ...
        elif pairing_method == 'dual_tower':
            if dual_tower is None:
                raise ValueError("pairing_method='dual_tower' requires --dual_tower_ckpt (and model loaded).")
            x0_paired, x1_paired = get_paired_samples_dual_tower(x0, x1, dual_tower=dual_tower)
        elif pairing_method == 'efm_closed_form':
            # EFM：xt 用随机配对，ut 用 softmax 加权所有样本
            x0_paired, x1_paired = get_paired_samples_cfm(x0, x1)
        elif pairing_method == 'efm_ot':
            # EFM+OT：xt 用 OT 配对，ut 仍用 softmax 加权所有样本
            x0_paired, x1_paired = get_paired_samples_otcfm(x0, x1)
        elif pairing_method == 'efm_down2_cfm':
            x0_paired, x1_paired = get_paired_samples_cfm(x0, x1)
        elif pairing_method == 'efm_down2_ot':
            x0_paired, x1_paired = get_paired_samples_otcfm(x0, x1)
        elif pairing_method == 'lsefm_blocks2_cfm':
            x0_paired, x1_paired = get_paired_samples_cfm(x0, x1)
        elif pairing_method == 'lsefm_blocks2_ot':
            x0_paired, x1_paired = get_paired_samples_otcfm(x0, x1)
        elif pairing_method == 'lsefm_blocks4_cfm':
            x0_paired, x1_paired = get_paired_samples_cfm(x0, x1)
        elif pairing_method == 'lsefm_blocks4_ot':
            x0_paired, x1_paired = get_paired_samples_otcfm(x0, x1)
        elif pairing_method == 'lsefm_blocks8_cfm':
            x0_paired, x1_paired = get_paired_samples_cfm(x0, x1)
        elif pairing_method == 'lsefm_blocks8_ot':
            x0_paired, x1_paired = get_paired_samples_otcfm(x0, x1)
        else:
        ...
        # 理论去噪方向：配对方法用 x1-x0；EFM 系用 softmax 加权所有样本的方向
        if pairing_method in ('efm_closed_form', 'efm_ot'):
            xt_flat = xt.reshape(batch_size, -1)
            x1_flat = x1.reshape(batch_size, -1)  # 用全部 x1，softmax 加权
            _, ut_theoretical, _ = efm_closed_form_weights_and_u(xt_flat, x1_flat, t, eps=1e-6)
            ut_theoretical = ut_theoretical.reshape_as(xt)
        elif pairing_method in ('efm_down2_cfm', 'efm_down2_ot'):
            _, ut_theoretical, _ = efm_closed_form_weights_and_u_downsample_2x(xt, x1, t, eps=1e-6)
        elif pairing_method in ('lsefm_blocks2_cfm', 'lsefm_blocks2_ot'):
            _, ut_theoretical = lsefm_block_weights_and_u(xt, x1, t, n_blocks=2, eps=1e-6)
        elif pairing_method in ('lsefm_blocks4_cfm', 'lsefm_blocks4_ot'):
            _, ut_theoretical = lsefm_block_weights_and_u(xt, x1, t, n_blocks=4, eps=1e-6)
        elif pairing_method in ('lsefm_blocks8_cfm', 'lsefm_blocks8_ot'):
            _, ut_theoretical = lsefm_block_weights_and_u(xt, x1, t, n_blocks=8, eps=1e-6)
        else:
...
def experiment(args):
    ...
    # Optional: load dual-tower encoder for pairing in embedding space
    dual_tower = None
    if getattr(args, 'dual_tower_ckpt', None):
        dual_ckpt_path = Path(args.dual_tower_ckpt)
        if not dual_ckpt_path.exists():
            raise FileNotFoundError(f"Dual-tower checkpoint not found: {dual_ckpt_path}")
        dual_ckpt = torch.load(dual_ckpt_path, map_location=device, weights_only=False)
        embed_dim = int(dual_ckpt.get('embed_dim', 256))
        dual_tower = DualTowerInfoNCE(embed_dim=embed_dim).to(device)
        dual_tower.load_state_dict(dual_ckpt['state_dict'])
        dual_tower.eval()
        print(f"Loaded dual-tower encoder from {dual_ckpt_path} (embed_dim={embed_dim})")

    # 定义配对方法：CFM、OTCFM、EFM(CFM/OT)、EFM 2x downsample、LSEFM block=2/4/8
    pairing_methods = [
        'otcfm',
        'efm_closed_form', 'efm_ot',
        'efm_down2_cfm', 'efm_down2_ot',
        'lsefm_blocks2_cfm', 'lsefm_blocks2_ot',
        'lsefm_blocks4_cfm', 'lsefm_blocks4_ot',
        'lsefm_blocks8_cfm', 'lsefm_blocks8_ot',
        'cfm',
    ]
    method_labels = {
        'cfm': 'CFM (Random)',
        'otcfm': 'OTCFM (OT)',
        'efm_closed_form': 'EFM (CFM pair)',
        'efm_ot': 'EFM (OT pair)',
        'efm_down2_cfm': 'EFM Down2x (CFM)',
        'efm_down2_ot': 'EFM Down2x (OT)',
        'lsefm_blocks2_cfm': 'LSEFM 2x2 (CFM)',
        'lsefm_blocks2_ot': 'LSEFM 2x2 (OT)',
        'lsefm_blocks4_cfm': 'LSEFM 4x4 (CFM)',
        'lsefm_blocks4_ot': 'LSEFM 4x4 (OT)',
        'lsefm_blocks8_cfm': 'LSEFM 8x8 (CFM)',
        'lsefm_blocks8_ot': 'LSEFM 8x8 (OT)',
    }
    colors = ['green', 'red', 'blue', 'cyan', 'magenta', 'orange', 'brown', 'pink', 'navy', 'olive', 'teal', 'coral']
        ...
        for batch_idx in range(args.num_batches):
            ...
            for pairing_method in pairing_methods:
                ...
                errors = compute_pairing_error_at_t(
                    ...
                    t_value, epsilon, metric=args.metric, dual_tower=dual_tower
                )
...
if __name__ == "__main__":
    ...
    # Output directory
    parser.add_argument('--output_dir', type=str, default='./exp/experiments_next_results',
                       help='Directory to save results')

    # Dual-tower pairing (InfoNCE)
    parser.add_argument(
        '--dual_tower_ckpt',
        type=str,
        default=None,
        help='Path to dual-tower encoder checkpoint produced by train_noise_image_infonce.py (--save_ckpt). Required if using pairing method "dual_tower".',
    )
