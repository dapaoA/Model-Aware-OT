"""
训练脚本：使用logit_normal和transport_logit_normal时间采样的OT-CFM方法
基于train.py，但使用不同的time_sampler
"""
import argparse
import time
...
def train(args):
    ...
    if resume_from:
        ...
        args.time_sampler = checkpoint_args.get('time_sampler', getattr(args, 'time_sampler', 'uniform'))
    ...
    model_name = f"{args.method}_{args.dataset}_{args.time_sampler}"
    ...
    writer.add_text("model/info", f"Method: {args.method}, Dataset: {args.dataset}, Time Sampler: {args.time_sampler}")
    ...
    # Create flow matcher with time_sampler
    sigma = args.sigma
    if args.method == 'sbcfm' and sigma <= 0:
        sigma = 0.5  # SB-CFM needs positive sigma
    ma_method = getattr(args, 'ma_method', 'downsample_2x')
    flow_matcher = create_flow_matcher(args.method, sigma, ma_method=ma_method, time_sampler=args.time_sampler)
    print(f"Flow matcher created with time_sampler: {args.time_sampler}")
    ...
    # Training loop
    if resume_from:
        print(f"Resuming training: {args.method} on {args.dataset} with time_sampler={args.time_sampler}")
    else:
        print(f"Starting training: {args.method} on {args.dataset} with time_sampler={args.time_sampler}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train OT-CFM with logit_normal time sampling")
    
    # Training method
    parser.add_argument('--method', type=str, default='otcfm',
                       choices=['cfm', 'otcfm', 'sbcfm', 'ma_otcfm', 'ma_tcfm', 'ma3_tcfm'],
                       help='Training method: cfm, otcfm, sbcfm, ma_otcfm, ma_tcfm, or ma3_tcfm')
    
    # Time sampler
    parser.add_argument('--time_sampler', type=str, default='logit_normal',
                       choices=['uniform', 'logit_normal', 'transport_logit_normal'],
                       help='Time sampling method: uniform, logit_normal, or transport_logit_normal')
    ...
    parser.add_argument('--vis_steps', type=int, default=50,
                       help='Number of steps for denoising visualization')
    parser.add_argument('--vis_step_interval', type=int, default=5,
                       help='Interval between steps shown in visualization')
